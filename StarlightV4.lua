...
function Starlight:CreateWindow(WindowSettings)
    ...
    -- Build folder path (root + folder if root exists, else just folder)
    local folderpath = root and (root .. "/" .. folder) or folder

    -- Ensure ThemesInRoot flag defaults to true if root is defined but flag is nil
    if root ~= nil then
        WindowSettings.FileSettings.ThemesInRoot =
            (WindowSettings.FileSettings.ThemesInRoot == nil) and true or WindowSettings.FileSettings.ThemesInRoot
    end

    -- Default NotifyOnCallbackError to true if not explicitly set
    if WindowSettings.NotifyOnCallbackError == nil then
        WindowSettings.NotifyOnCallbackError = true
    end

    -- Config autoload path (always under /configs/)
    Starlight.FileSystem.AutoloadConfigPath =
        string.format("%s/%s/configs/", Starlight.FileSystem.Folder, folderpath)

    -- Theme autoload path (root vs. folderpath depending on ThemesInRoot flag)
    if WindowSettings.FileSettings.ThemesInRoot then
        Starlight.FileSystem.AutoloadThemePath =
            string.format("%s/%s/themes/", Starlight.FileSystem.Folder, root)
    else
        Starlight.FileSystem.AutoloadThemePath =
            string.format("%s/%s/themes/", Starlight.FileSystem.Folder, folderpath)
    end
	...
	Starlight.Window = {
        ...
        TabSize = WindowSettings.TabSize or 210,
	...
	do
		...
		mainWindow.Sidebar.Player.Header.Text = "User"--Player.DisplayName
		mainWindow.Sidebar.Player.subheader.Text = "ESAN-PROTECT" --Player.Name
        --print(mainWindow.Sidebar.Size) -- DEF: 210
        local ActuallySize = Starlight.Window.TabSize
        mainWindow.Sidebar.Size = UDim2.new(0, ActuallySize, 1, 0)
        mainWindow.Content.Size = UDim2.new(1, -ActuallySize, 1, 0)
        mainWindow.Content.Position = UDim2.new(0, ActuallySize, 0, 0)
		...
		if WindowSettings.LoadingEnabled then
			...
			title.playerName.Text = "User"--Player.DisplayName
	...
	function Starlight.Window:CreateTabSection(Name :string, Visible)
		...
		function TabSection:CreateTab(TabSettings, TabIndex)
					...
					task.spawn(function()
						...
						function Element:Set(NewElementSettings , NewIndex)
							...
							for i, ElementInstance in pairs(Instances) do
                                ...
                                local accent = ElementInstance.PART_Backdrop:FindFirstChild("Accent")
				...
				function Groupbox:CreateLabel(ElementSettings, Index)
					...
					function Element:AddDropdown(NestedSettings, NestedIndex, Parent, ParentIndex)
						...
						task.spawn(function()
							...
							local function Refresh()
                                local DropdownFrame = NestedElement.Instances[2]
                                if DropdownFrame then
                                    local List = DropdownFrame:FindFirstChild("List")
                                    if List then
								...
								for _, option in ipairs(NestedElement.Values.Options) do
                                    if Resources:FindFirstChild("Elements") then
			...
			function Tab:BuildThemeGroupbox(Column, Style, ButtonsCentered)
				...
				local themesPath = Starlight.FileSystem.AutoloadThemePath -- WindowSettings.FileSettings.ThemesInRoot and `{Starlight.FileSystem.Folder}/{root}/themes` or `{Starlight.FileSystem.Folder}/{folderpath}/themes`
				...
				instance:CreateButton({
                    ...
                    Callback = function()
                        ...
                        if not success then
                            ...
                            Starlight:Notification({
                                Title = "Save Failed",
                                Icon = 6031082533, -- ⚠️ warning icon
                                Content = string.format("Could not save autoload theme.\nError: %s", tostring(err)),
                            })
                            return
                        end
...
function Starlight.FileSystem:RefreshConfigList(path)
	...
	local list = isfolder(path) and listfiles(path) or {}
...
-- if getgenv().TestUI and enabled then
...
-- 		LoadingSettings = {
-- 			Title = "ESAN HUB",
-- 			Subtitle = "Made by Kenei and Zanexywex",
-- 		},
...
--         TabSize = 300,
--         DefaultSize = UDim2.fromOffset(1000,450),
